...
namespace Server.Poker
{
    ...
    public class PokerPersistence : Item
    {
        public override string DefaultName { get { return "Poker Persistence - Internal"; } }

        private static PokerPersistence m_Instance;

        public static PokerPersistence Instance { get { return m_Instance; } }
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            writer.Write(GameBackup.PokerGames.Count);
            foreach(var game in GameBackup.PokerGames)
            {
                writer.Write(game.Players.Count);
                foreach(var player in game.Players.Players)
                {
                    writer.Write(player.Mobile);
                    writer.Write(player.Gold + player.RoundBet);
                }
            }
        }
    }
