...
namespace Server.Items
{
    ...
    public class PirateSpyglass : SpyglassMapViewItem
    {
        ...
        public void ResolveSpyglassAction(PlayerMobile player)
        {
            ...
            {
                if (player.BoatOccupied.IsFriend(player) || player.BoatOccupied.IsCoOwner(player) || player.BoatOccupied.IsOwner(player))
                {
                    if (player.BoatOccupied.CheckForUpgrade(typeof(CrowsNestUpgrade)))
                        rangeBonus += .25;
                }
            }                

            int modifiedViewDistance = (int)((double)m_ViewDistance * rangeBonus * spyglassActionRangeModifer);
            ...
            if (searchForShips)
            {
                foreach (BaseBoat foundBoat in BaseBoat.AllBoatInstances)
                {
                    if (foundBoat == null)
                        continue;
    ...
    public class PirateSpyglassGump : Gump
    {
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                ...
                case 6:
                    if (m_Ships)
                        m_Player.SpyglassAction = (SpyglassAction)(m_Range + 1);
                    else
                        m_Player.SpyglassAction = (SpyglassAction)(m_Range + 4);
                    ...
                    m_Player.NextSpyglassActionAllowed = DateTime.UtcNow + TimeSpan.FromSeconds(duration);
