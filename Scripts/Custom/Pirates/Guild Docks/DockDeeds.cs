    ...
    public class DockDeed : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else if (from.AccessLevel >= AccessLevel.GameMaster)
            {
                ...
                from.SendMessage("Your guild must have a pirate stone to place guild docks.");
            
            else
            {
                PirateStone.m_PirateStoneDictionary.TryGetValue((Guild)from.Guild, out m_Stone);

                if (!from.InRange(m_Stone.Location, 25))                
                    from.SendMessage("You must be within 25 tiles of your pirate stone to place guild docks.");
