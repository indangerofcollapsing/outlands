	...
	public class XmlDrag : XmlAttachment
	{
		...
		private class InternalTimer : Timer
		{
			...
			protected override void OnTick()
			{
                ...
                if (draggedby != null)
                    draggedby.RevealingAction();

                if (parent is BaseSiegeWeapon)
                {
                    if (((BaseSiegeWeapon)parent).Hits <= 0)
                    {
                        m_attachment.DraggedBy = null;
                        if (draggedby is PlayerMobile)
                            ((PlayerMobile)draggedby).IsDragging = false;
                        Stop();
                        return;
                    }
                }
				...
				if (newloc != m_attachment.CurrentLoc)
				{
					m_attachment.CurrentLoc = newloc;
                    //m_attachment.CurrentMap = newmap; // don't change maps
