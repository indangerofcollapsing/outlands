	...
	public class SiegeComponent : AddonComponent
	{
		...
		private class ReleaseEntry : ContextMenuEntry
		{
			...
			public override void OnClick()
			{
				...
				{
                    if (m_drag.DraggedBy is PlayerMobile)
                        ((PlayerMobile)m_drag.DraggedBy).IsDragging = false;
		...
		private class ConnectEntry : ContextMenuEntry
		{
			...
			public override void OnClick()
			{
				if (m_drag != null && m_from != null && m_drag.DraggedBy == null)
				{
                    if (m_from is PlayerMobile)
                    {
                        m_drag.DraggedBy = m_from;
                        ((PlayerMobile)m_from).IsDragging = true;
                    }
                    //m_from.SendMessage("Select a mobile to drag the weapon");
                    //m_from.Target = new DragTarget(m_drag);
                }
                else if (m_drag.DraggedBy != null && m_from != null)
                {
                    m_from.SendMessage("Someone is already dragging this.");
                }
			}
		}
