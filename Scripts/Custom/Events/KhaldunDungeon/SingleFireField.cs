...
{
	public class SingleFireField : Item
	{
		...
		[Constructable]
        public SingleFireField(Mobile owner, int hue, double intervalDuration, int maxIntervals, int minDamage, int maxDamage, bool hitOwner, bool hitMonsters, bool hitPlayers, int hitSound, bool activateOnMoveOver): base(0x398C)		
		{            
            Name = "fire field";
            Hue = hue;
        ...
        public SingleFireField(Serial serial): base(serial)
		{	
		}
        ...
        public override bool OnMoveOver(Mobile mobile)
        {
            ...
            Effects.PlaySound(mobile.Location, mobile.Map, 0x208); 
		...
		{
            public SingleFireField m_SingleFireField;
            ...
            public InternalTimer(SingleFireField singleFireField, Mobile owner, double intervalDuration, int maxIntervals, int minDamage, int maxDamage, bool hitOwner, bool hitMonsters, bool hitPlayers, int hitSound): base(TimeSpan.FromSeconds(0), TimeSpan.FromSeconds(intervalDuration))
			{
				Priority = TimerPriority.TwoFiftyMS;

                m_SingleFireField = singleFireField;
			...
			{
                if (m_SingleFireField == null)
                {
                    Stop();
                    return;
                }

                if (m_SingleFireField.Deleted)
                {
                    Stop();
                    return;
                }
                ...
                {
                    Stop();
                    m_SingleFireField.Delete();

                    return;
                }
                
                IPooledEnumerable mobilesOnTile = m_SingleFireField.GetMobilesInRange(0);
