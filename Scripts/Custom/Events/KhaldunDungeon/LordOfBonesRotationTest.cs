...
namespace Server.Mobiles
{
	...
	public class LordOfBonesRotationTest : BaseCreature
	{
        ...
        public int m_RotationTick = 0;
        private Timer m_Timer;        

        public override Loot.LootTier LootTier { get { return Loot.LootTier.Eight; } }

		[Constructable]
		public LordOfBonesRotationTest() : base( AIType.AI_Melee, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
            Name = "Lord of Bones";
            ...
            Timer.DelayCall(TimeSpan.FromMilliseconds(50), delegate { BuildRotatingComponents(); });
		}
        ...
        public override void SetUniqueAI()
        {
            ...
            damageIntervalThreshold = (int)Math.Ceiling((double)HitsMax / (double)totalIntervals);
        ...
        public void BuildRotatingComponents()
        {
            ...
            for (int a = 0; a < m_ItemIDs.Count; a++)
            {
                Static item = new Static(m_ItemIDs[a]);

                Point3D startingLocation = Location;
                startingLocation.Z = Location.Z + 13;

                SpellHelper.AdjustField(ref startingLocation, Map, 12, false);

                item.MoveToWorld(startingLocation, Map);
                m_Items.Add(item);
            }
        ...
        public void UpdateItemPosition(Item item, int position)
        {
            if (m_Items.Count == 0)
                return;
            ...
            {
                case 1: positionOffset = 0; break;
                case 2: positionOffset = 4; break;
                case 3: positionOffset = 3; break;
                case 4: positionOffset = 2; break;               
            }

            if (m_Items.Count >= 4)
                positionOffset = 1;

            int directionValue = m_RotationTick + (position * positionOffset);

            if (directionValue >= 8)            
                directionValue = directionValue - 8;            

            Direction directionFromMobile = DirectionFromMobile(directionValue);
            Point3D newLocation = Location;

            switch (directionFromMobile)
            {
                case Direction.North: newLocation = SpecialAbilities.GetPointByDirection(Location, Direction.Right); break;
                case Direction.Right: newLocation = SpecialAbilities.GetPointByDirection(Location, Direction.East); break;
                case Direction.East: newLocation = SpecialAbilities.GetPointByDirection(Location, Direction.Down); break;
                case Direction.Down: newLocation = SpecialAbilities.GetPointByDirection(Location, Direction.South); break;
                case Direction.South: newLocation = SpecialAbilities.GetPointByDirection(Location, Direction.Left); break;
                case Direction.Left: newLocation = SpecialAbilities.GetPointByDirection(Location, Direction.West); break;
                case Direction.West: newLocation = SpecialAbilities.GetPointByDirection(Location, Direction.Up); break;
                case Direction.Up: newLocation = SpecialAbilities.GetPointByDirection(Location, Direction.North); break;
            }
                        
            newLocation.Z = Location.Z + 13;
        ...
        public Direction DirectionFromMobile(int directionValue)
        {
            ...
            switch (directionValue)
            {
                ...
                case 5: direction = Direction.Left; break;
                case 6: direction = Direction.West; break;
                case 7: direction = Direction.Up; break;
            }
        ...
        {
            private LordOfBonesRotationTest m_LordOfBonesTest;

            public InternalTimer(LordOfBonesRotationTest lordOfBonesTest) : base(TimeSpan.Zero, TimeSpan.FromMilliseconds(110))
            {
                Priority = TimerPriority.TwentyFiveMS;

                m_LordOfBonesTest = lordOfBonesTest;
            }

            protected override void OnTick()
            {
                if (m_LordOfBonesTest == null)
                    Stop();

                if (m_LordOfBonesTest.Deleted)
                    Stop();

                m_LordOfBonesTest.m_RotationTick++;

                if (m_LordOfBonesTest.m_RotationTick == 8)
                    m_LordOfBonesTest.m_RotationTick = 0;

                for (int a = 0; a < m_LordOfBonesTest.m_Items.Count; a++)
                {
                    Item item = m_LordOfBonesTest.m_Items[a];

                    if (item != null)
                    {
                        if (!item.Deleted && item.Map == m_LordOfBonesTest.Map)
                            m_LordOfBonesTest.UpdateItemPosition(item, a);
                    }
                }
            }
        }
        ...
        public LordOfBonesRotationTest(Serial serial): base(serial)
		{
		}
