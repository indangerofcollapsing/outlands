...
namespace Server.Gumps
{
    public class TitlesGump : Gump
    {
            ...
            : base(200, 50)
        {
            ...
            // Add faction title option if any such is gained
            Custom.Townsystem.PlayerState ps = Custom.Townsystem.PlayerState.Find(from);
            Custom.Townsystem.Town town = Custom.Townsystem.Town.CheckCitizenship(from);

            if (ps != null && ps.Faction != null)
            {
                m_Prefixes.Add(ps.GetTitle());
            }
            ...
            for (int i = 0; i < countPrefix; i++)
            {
                ...
                bool isPaladinTitle = false;

                foreach (String title in PlayerClassPersistance.PaladinTitles)
                {
                    if (title == m_Prefixes[index])
                    {
                        isPaladinTitle = true;
                        break;
                    }
                ...
                bool isMurdererTitle = false;

                foreach (String title in PlayerClassPersistance.MurdererTitles)
                {
                    if (title == m_Prefixes[index])
                    {
                        isMurdererTitle = true;
                        break;
                    }
                }

                //Currently Unavailable Titles
                if ((!pm.Paladin && isPaladinTitle) || (!pm.Murderer && isMurdererTitle))
                {
                    this.AddLabel(64, 155 + i * 25, 0, String.Format("{0}. {1}", i + 1 + m_PageNoPrefix * 10, m_Prefixes[index]));
                    buttonID++;
                }
