...
namespace Server.Gumps
    {
    public class AllPlayersGump : Gump
        {
        public static void Initialize()
            {
            CommandSystem.Register( "AllPlayers", AccessLevel.Developer, new CommandEventHandler( AllPlayers_OnCommand ) );
            }

        public static string m_Args;

        [Usage( "AllPlayers" )]
        [Description( "Lists all mobiles." )]
        private static void AllPlayers_OnCommand( CommandEventArgs e )
            {
            e.Mobile.SendGump( new AllPlayersGump( e.Mobile ) );
            }
        ...
        public AllPlayersGump( Mobile owner ) : this( owner, BuildList( owner ), 0 )
            {
            }

        public AllPlayersGump(Mobile owner, List<Mobile> items, int page)
            : base(GumpOffsetX, GumpOffsetY)
            {
                owner.CloseGump(typeof(AllPlayersGump));
        ...
        public void Initialize( int page )
            ...
            for( int i = 0, index = page * EntryCount; i < EntryCount && index < m_Mobiles.Count; ++i, ++index )
                ...
                AddLabelCropped( x + TextOffsetX, y, EntryWidth - TextOffsetX, EntryHeight, mobile is BaseCreature && ( (BaseCreature)mobile ).IsStabled ? 33 : 0x58 /*hue*/,
                    mobile.Deleted ? "(deleted)" : ( string.Format( "Account:{0}, Name:{1}.  {2}:{3}", ( (Account)mobile.Account ).Username, mobile.Name, mobile.Map, mobile.Location)));
            ...
            if( WipeLabel )
                AddLabel( x + WipeLabelOffsetX, y + WipeLabelOffsetY, TextHue, "Wipe All Players (THIS WOULD BE VERY STUPID)" );
            }
        ...
        public override void OnResponse( NetState state, RelayInfo info )
            ...
            switch( info.ButtonID )
                ...
                case 1: // Previous 
                    {
                    if( m_Page > 0 )
                        from.SendGump( new AllPlayersGump( from, m_Mobiles, m_Page - 1 ) );
                ...
                default:
                    ...
                    if( index >= 0 && index < m_Mobiles.Count )
                        ...
                        if( s.Deleted )
                            ...
                            from.SendGump( new AllPlayersGump( from, m_Mobiles, m_Page ) );
                            }
                        else
                            {
                            if( deleting )
        ...
        private class DeleteGump : Gump
            ...
            public override void OnResponse( Server.Network.NetState sender, RelayInfo info )
                ...
                m_From.SendGump( new AllPlayersGump( m_From, m_Mobiles, 0 ) );
