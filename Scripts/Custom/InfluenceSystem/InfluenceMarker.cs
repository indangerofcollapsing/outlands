        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            if (info.ButtonID == 2)
            {
                ...
                if (bankBalance >= investmentNextLevelCost)
                {
                    ...
                    //Add Investment into Town's Treasury
                    Server.Custom.Townsystem.Town town = Server.Custom.Townsystem.Town.Parse(m_Marker.MarkerRegion.ToString());

                    if (town == null)                    
                        town = Server.Custom.Townsystem.Town.FromRegion(pm_From.Region);

                    if (town != null)                    
                        town.Treasury += investmentNextLevelCost;                    
                }
