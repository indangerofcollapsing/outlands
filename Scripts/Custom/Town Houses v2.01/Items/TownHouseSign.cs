	...
	public class TownHouseSign : Item
	{
		...
		public void Purchase( Mobile m, bool sellitems )
		{
            try
            {
                ...
                if (m.AccessLevel == AccessLevel.Player && !WithdrawOgreCoin(m, price))
                {
                ...
                if (m.AccessLevel == AccessLevel.Player)
                    m.SendMessage(String.Format("{0} dragon coins have been withdrawn from your bank box.", price.ToString())); // ~1_AMOUNT~ ogre coin has been withdrawn from your bank box.
        ...
        private static bool WithdrawOgreCoin(Mobile from, int amount)
        {
            int balance = 0;
            Item[] coin;
            ...
            if (bank != null)
            {
                coin = bank.FindItemsByType(typeof(DragonCoin));

                for (int i = 0; i < coin.Length; ++i)
                    balance += coin[i].Amount;
            }
            else
            {
                coin = new Item[0];
            ...
            for (int i = 0; amount > 0 && i < coin.Length; ++i)
            {
                if (coin[i].Amount <= amount)
                {
		...
		private void RentDue()
		{
			...
			if ( !c_Free && c_House.Owner.AccessLevel == AccessLevel.Player && !WithdrawOgreCoin( c_House.Owner, c_Price ) )
			{
			...
			if ( !c_Free )
				c_House.Owner.SendMessage( "The bank has withdrawn {0} dragon coins rent for your town house.", c_Price );
		...
		public bool CanBuyHouse( Mobile m )
		{
			...
			if ( c_Murderers == Intu.Yes && m.ShortTermMurders < 5 )
				return false;

			if ( c_Murderers == Intu.No && m.ShortTermMurders >= 5 )
				return false;
