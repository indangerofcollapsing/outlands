		...
		*/
		...
		public WayPoint WayPoint
		{
            ...
            { 
                ...
                if (m_WayPoint != null)
                {
                    if (m_WayPoint.XMLSpawners != null)
                    {
                        if (!m_WayPoint.XMLSpawners.Contains(this))
                            m_WayPoint.XMLSpawners.Add(this);
                    }                    

                    //Match Waypoint Visibility and Hue to XMLSpawner's Current Status
                    WayPoint checkWaypoint = m_WayPoint;
                    checkWaypoint.Hue = Hue;

                    if (Visible)
                        checkWaypoint.Visible = true;                      
                    
                    List<WayPoint> m_WaypointSequence = new List<WayPoint>();

                    while (checkWaypoint != null)
                    {
                        if (checkWaypoint.NextWaypoint != null)
                        {
                            if (m_WaypointSequence.Contains(checkWaypoint.NextWaypoint))
                                break;
                            
                            checkWaypoint = checkWaypoint.NextWaypoint;
                            checkWaypoint.Hue = Hue;

                            if (Visible)
                                checkWaypoint.Visible = true;

                            m_WaypointSequence.Add(checkWaypoint);
                        }
        ...
        public override int Hue
        {
            ...
            set
            {
                ...
                {
                    //Match Waypoint Visibility and Hue to XMLSpawner's Current Status
                    WayPoint checkWaypoint = WayPoint;
                    checkWaypoint.Hue = Hue;
		...
		public override void OnSingleClick(Mobile from)
		{
            ...
            if (m_WayPoint != null)
            {
                LabelTo(from, "Waypoint: " + m_WayPoint.Location.X.ToString() + "," + m_WayPoint.Location.Y.ToString() + "," + m_WayPoint.Location.Z.ToString());
            }
		}
		...
		public override void OnDelete()
		{
            ...
            //Remove This From WayPoint's List of Connected XMLSpawners
            if (this.m_WayPoint != null)
            {
                if (m_WayPoint.XMLSpawners.IndexOf(this) >= 0)
                {
                    m_WayPoint.XMLSpawners.Remove(this);
                }
            }
		}
...
public static void _TraceEnd(int index)
{
	   if(index < MaxTraces){
		...
		public static void Initialize()
		{
			...
			CommandSystem.Register("ShowAllXMLSpawners", AccessLevel.Administrator, new CommandEventHandler(ShowSpawnPoints_OnCommand));
            CommandSystem.Register("HideAllXMLSpawners", AccessLevel.Administrator, new CommandEventHandler(HideSpawnPoints_OnCommand));
							...
							if (o is Mobile)
							{
								...
								*/
								...
								if (m is BaseCreature)
								{
                                    ...
                                    if (c.AIObject != null)
                                    {
                                        c.AIObject.SetWaypointTimeout(c);
                                    }
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			switch (version)
			{
                case 33:
                    {
                        m_Hotspot = (Custom.Hotspot)reader.ReadItem();
                        goto case 32;
                    }
