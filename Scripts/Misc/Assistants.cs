...
namespace Server.Misc
{
	public static partial class Assistants
	{
		private static class Settings
		{
			...
			{
				//DisallowFeature(Features.RandomTargets);
				DisallowFeature(Features.ClosestTargets);
				DisallowFeature(Features.EnemyTargetShare);
				DisallowFeature(Features.SpellTargetShare);
				DisallowFeature(Features.AutoBandage);
				DisallowFeature(Features.AutolootAgent);
				DisallowFeature(Features.HumanoidHealthChecks);
				DisallowFeature(Features.SpeechJournalChecks);
			}
			...
			public enum Features : ulong
			{
				...
				HumanoidHealthChecks = 1 << 25, // Humanoid (not self) health checks via macros
				SpeechJournalChecks = 1 << 26, // Journal speech (party, guild, etc) checks via macros, disable to allow only system message checks
				All = ulong.MaxValue
			}
		...
		private static class Negotiator
		{
			...
			private static void EventSink_Login(LoginEventArgs e)
			{
				...
				{
					if (m is PlayerMobile && ((PlayerMobile)m).Sallos)
						return;
