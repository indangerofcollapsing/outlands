...
namespace Server.Misc
{
	public class CrashGuard
	{
		...
		public static void Initialize()
		{
			...
			if (Enabled)
				CommandSystem.Register("CrashServerNow", AccessLevel.GameMaster, new CommandEventHandler(CrashServerNow_OnCommand));
		}

		[Usage("CrashServerNow")]
		public static void CrashServerNow_OnCommand(CommandEventArgs e)
		{
			Item dummy = null;
			dummy.Name = "Banana"; //poof
		}
		...
		private static void SendEmail( string filePath )
		{
			...
			MailMessage message = new MailMessage("ipycrashes@gmail.com", Email.CrashAddresses);

			message.Subject = "Automated IPY Crash Report";

			message.Body = "Automated IPY Crash Report. See attachment for details.";
		...
		private static void GenerateCrashReport( CrashedEventArgs e )
		{
			...
			try
			{
				...
				using ( StreamWriter op = new StreamWriter( filePath ) )
				{
					...
					op.WriteLine( "Server Time (utc): {0}", DateTime.UtcNow );
					op.WriteLine( "Local Time: {0}", DateTime.UtcNow );
				...
				if (Email.CrashAddresses != null && !System.Diagnostics.Debugger.IsAttached)
					SendEmail( filePath );
			}
			catch
			{
