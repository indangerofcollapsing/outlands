...
namespace Server.Guilds
{
    ...
    {
        [Description("Increased Karma and Fame")]
        IncreasedFameKarma,
        [Description("Less Damage To Weapons")]
        LessWeaponDamage,
        [Description("Less Damage To Armor")]
        LessArmorDamage,
        [Description("Guildmate Resurrection")]
        RezGuildmate,
        [Description("Self Resurrection")]
        SelfRez,
        [Description("Dungeon Boss Restart")]
        SequentialRestart,
        [Description("Area Resurrection")]
        AoeRez
    }
    #endregion
    ...
    public class ExperienceTimer : Timer
    {
        public ExperienceTimer() : base(TimeSpan.FromMinutes(1), TimeSpan.FromMinutes(1))
        {
        ...
        {
            if (DateTime.Today.Date > GuildExperienceResetPersistence.Instance.LastUpdate.Date)
            {
                foreach (Guild g in Guild.List.Values)
                    g.ExperienceEarnedToday = 0;

                GuildExperienceResetPersistence.Instance.LastUpdate = DateTime.Now;
            }
        }
    }
    ...
    public class GuildExperienceResetPersistence : Item
    {
        public static void Initialize()
        {
            new ExperienceTimer().Start();
        }

        private static GuildExperienceResetPersistence m_Instance;

        public static GuildExperienceResetPersistence Instance
        {
            get
            {
                if (m_Instance == null || m_Instance.Deleted)
                    new GuildExperienceResetPersistence();
        ...
        public override string DefaultName { get { return "Guild Daily Experience Reset Persistance - Internal"; } }

        public GuildExperienceResetPersistence()
            : base(0)
        {
            Movable = false;
            Visible = false;
        ...
        public GuildExperienceResetPersistence(Serial serial)
            : base(serial)
        {
            m_Instance = this;
        }
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            int version = reader.ReadInt();
            LastUpdate = reader.ReadDateTime();
        }
    }
    ...
    public class Guild : BaseGuild
    {
        ...
        public static void Configure()
        {
            ...
            CommandSystem.Register("ResetGuildDailyLimit", AccessLevel.Administrator, new CommandEventHandler(ResetGuildDailyLimit_OnCommand));
        }

        private static void ResetGuildDailyLimit_OnCommand(CommandEventArgs e)
        {
            foreach (Guild guild in Guild.List.Values)
                guild.ExperienceEarnedToday = 0;
        }
        ...
        public static bool NewGuildSystem { get { return true; /* Core.SE*/; } }

        public static readonly int RegistrationFee = 12000;
        ...
        public static readonly TimeSpan MinimumMemberTimeForAbilities = TimeSpan.FromDays(3);

        private static readonly Dictionary<GuildBonus, GuildLevel> BonusUnlockLevel = new Dictionary<GuildBonus, GuildLevel>()
        {
            { GuildBonus.IncreasedFameKarma, GuildLevel.One },
            { GuildBonus.LessWeaponDamage,   GuildLevel.Two },
            { GuildBonus.LessArmorDamage,    GuildLevel.Three },
            { GuildBonus.RezGuildmate,       GuildLevel.Five },
            { GuildBonus.SequentialRestart,  GuildLevel.Eight },
            { GuildBonus.SelfRez,            GuildLevel.Nine },
            { GuildBonus.AoeRez,             GuildLevel.Ten },
        };

        // must contain same keys as last usage
        private static readonly Dictionary<GuildBonus, TimeSpan> BonusCooldowns = new Dictionary<GuildBonus, TimeSpan>() 
        {
            { GuildBonus.RezGuildmate,      TimeSpan.FromMinutes(30) },
            { GuildBonus.SelfRez,           TimeSpan.FromMinutes(30) },
            { GuildBonus.SequentialRestart, TimeSpan.FromHours(6)    },
            { GuildBonus.AoeRez,            TimeSpan.FromHours(24)   },
        };

        // must contain same keys as bonus cooldowns
        public Dictionary<GuildBonus, DateTime> BonusLastUsage = new Dictionary<GuildBonus, DateTime>() 
        {
            { GuildBonus.RezGuildmate,      DateTime.UtcNow },
            { GuildBonus.SelfRez,           DateTime.UtcNow },
            { GuildBonus.SequentialRestart, DateTime.UtcNow },
            { GuildBonus.AoeRez,            DateTime.UtcNow },
        };

        public static readonly Dictionary<GuildLevel, int> UnlockPrices = new Dictionary<GuildLevel, int>()
        {
            { GuildLevel.Zero,  0     },
            { GuildLevel.One,   10000 },
            { GuildLevel.Two,   20000 },
            { GuildLevel.Three, 30000 },
            { GuildLevel.Four,  40000 },
            { GuildLevel.Five,  50000 },
            { GuildLevel.Six,   60000 },
            { GuildLevel.Seven, 70000 },
            { GuildLevel.Eight, 80000 },
            { GuildLevel.Nine,  90000 },
            { GuildLevel.Ten,   100000 },
        };

        public static readonly Dictionary<GuildLevel, int> ExperienceLevels = new Dictionary<GuildLevel, int>()
        {
            { GuildLevel.Zero,  50 },
            { GuildLevel.One,   60 },
            { GuildLevel.Two,   70 },
            ...
            { GuildLevel.Five,  100 },
            { GuildLevel.Six,   110 },
            { GuildLevel.Seven, 120 },
            { GuildLevel.Eight, 130 },
            { GuildLevel.Nine,  140 },
            { GuildLevel.Ten,   150 },
        };

        private GuildLevel m_Level = GuildLevel.Zero;
        ...
        public int Experience
        {
            ...
            set 
            {
                if (value > ExperienceLevels[m_Level])
                    value = ExperienceLevels[m_Level];

                m_Experience = value; 
            }
        }

        private Int32 m_Gold = 0;
        ...
        private static int ExperiencePerDay = 25;
        public int ExperienceEarnedToday = 0;
        #endregion
        ...
        public void TickExperience(int amount = 1)
        {
            var updated = m_Experience + amount;
            if (updated > ExperienceLevels[m_Level])
                updated = ExperienceLevels[m_Level];

            if (ExperienceEarnedToday >= ExperiencePerDay)
            {
                GuildTextMessage(33, "Your guild has earned the maximum experience for today.");
                return;
            }
            else
            {
                m_Experience = updated;
                ExperienceEarnedToday++;
            }

            if (m_Experience == ExperienceLevels[m_Level])
            {
                GuildTextMessage(1153, "Your guild has earned enough experience to unlock the next guild level!");
            }
            else
            {
                GuildTextMessage("Your guild has earned experience!");
            }
        }

        public bool CanPurchaseUnlock()
        {
            if (m_Level == GuildLevel.Ten)
                return false;

            int cost = UnlockPrices[m_Level + 1];
            int expReq = ExperienceLevels[m_Level];

            return m_Gold >= cost && m_Experience >= expReq;
        }
        ...
        public void PurchaseUnlock()
        {
            ...
            {
                m_Gold -= cost;
                GuildTextMessage(1153, "Your guild has advanced to the next level!");
                m_Level++;
                m_Experience = 0;
            }
        }

        public bool HasBonus(GuildBonus bonus)
        {
            return m_Level >= BonusUnlockLevel[bonus];
        }

        // proc rate for passive bonuses
        public double BonusChance(GuildBonus bonus)
        {
            switch (bonus)
            {
                case GuildBonus.LessArmorDamage:
                    return 0.1;
                    break;
                case GuildBonus.LessWeaponDamage:
                    return 0.1;
                    break;
                default:
                    return 0.00;
        ...
        public bool CanUseAbility(Mobile from, GuildBonus bonus)
        {
            if (!HasBonus(bonus))
                return false;

            if (from.GuildJoinTime > DateTime.UtcNow + MinimumMemberTimeForAbilities)
                return false;

            return DateTime.UtcNow > BonusLastUsage[bonus] + BonusCooldowns[bonus];
        }

        public void AbilityUsed(Mobile from, GuildBonus bonus)
        {
            if (!HasBonus(bonus) || !CanUseAbility(from, bonus))
                return;

            BonusLastUsage[bonus] = DateTime.UtcNow;
        }
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            // version 9 exp daily cap
            writer.Write(ExperienceEarnedToday);

            // version 8 levels
            writer.Write(BonusLastUsage.Count);
            foreach (var kvp in BonusLastUsage)
            {
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            {
                case 9:
                    ExperienceEarnedToday = reader.ReadInt();
                    goto case 8;
                case 8:
                    {
                        ...
                        {
                            BonusLastUsage[(GuildBonus)reader.ReadInt()] = reader.ReadDateTime();
                        }

                        m_Level = (GuildLevel)reader.ReadInt();
        ...
        public void GuildChat(Mobile from, string text)
        {
            ...
            if (text == "list")
            {
                string strList = "<BASEFONT COLOR=WHITE>GUILD MEMBERS ONLINE<br><br>";
                int count = 0;
                ...
                for (int i = 0; i < m_Members.Count; i++)
                {
                    ...
                    if (state != null)
                        strList = String.Concat(strList, String.Format("{0}. {1}<br>", ++count, m.RawName));
                }
        ...
        private class OnlineMembersGump : Server.Gumps.Gump
        {
            public OnlineMembersGump(string strHTML)
                : base(100, 100)
            {
                this.Closable = true;
