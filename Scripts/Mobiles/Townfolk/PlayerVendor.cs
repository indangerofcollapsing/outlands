...
namespace Server.Mobiles
{
	...
	public class VendorItem
	{
		...
		public bool IsForSale{ get{ return Price >= 0 || NonCitizenPrice >= 0; } }
        public bool IsForFree { get { return !IsForSale; } }

		public bool Valid{ get{ return m_Valid; } }

		public VendorItem( Item item, int price, int nonCitizenPrice, string description, DateTime created)
		{
			m_Item = item;
			m_Price = price;
            m_NonCitizenPrice = nonCitizenPrice;
	...
	public class VendorBackpack : Backpack
	{
		...
		public override void GetChildNameProperties( ObjectPropertyList list, Item item )
		{
			...
			if ( !vi.IsForSale )
				...
				list.Add( 1043304, String.Format("Price (w/ tax): {0}", (int)(vi.Price*(1+tax)))  ); // Price: ~1_COST~
		}
			...
			if ( RootParent is PlayerVendor )
			{
				...
				if ( vi != null )
				{
                    if (!vi.IsForSale)
                        ...
                        item.LabelTo(from, 1043304, vi.GetPrice(from,vendor).ToString()); // Price: ~1_COST~
                    else
                        item.LabelTo(from, String.Format("Price (w/ tax): {0}", (int)(vi.GetPrice(from,vendor))));
	...
	public class PlayerVendor : Mobile
	{
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			foreach ( VendorItem vi in m_SellItems.Values )
			{
                ...
                writer.Write((int)vi.NonCitizenPrice);
			...
			switch ( version )
			{
                ...
                case 1:
				{
					...
					for ( int i = 0; i < count; i++ )
					{
                        ...
                        int nonCitizenPrice = reader.ReadInt();
                        if ( nonCitizenPrice > 100000000)
                            nonCitizenPrice = 100000000;
						...
						if ( item != null )
						{
							SetVendorItem( item, version < 1 && price <= 0 ? -1 : price, nonCitizenPrice, description, created );
						}
					}
		...
		private VendorItem SetVendorItem( Item item, int price, int nonCitizenPrice, string description )
		{
			return SetVendorItem( item, price, nonCitizenPrice, description, DateTime.UtcNow );
		}

		private VendorItem SetVendorItem( Item item, int price, int nonCitizenPrice, string description, DateTime created )
		{
			RemoveVendorItem( item );

			VendorItem vi = new VendorItem( item, price, nonCitizenPrice, description, created );
			m_SellItems[item] = vi;
		...
		public static void TryToBuy( Item item, Mobile from )
		{
			...
			else
			{
				...
				from.SendGump( new PlayerVendorBuyGump( from, vendor, vi ) );
			}
		}
		...
		private class VendorPricePrompt : Prompt
		{
			...
			public override void OnResponse( Mobile from, string text )
			{
				...
				SetInfo( from, price, price, Utility.FixHtml( description ) );
			}
			...
			private void SetInfo( Mobile from, int price, int nonCitizenPrice, string description )
			{
				Item item = m_VI.Item;

				bool setPrice = false;

				if ( price < 0 && nonCitizenPrice < 0 ) // Not for sale
				{
					price = -1;
                    nonCitizenPrice = -1;
				...
				if ( setPrice )
				{
					m_Vendor.SetVendorItem( item, price, price, description );
				}
				else
				{
        ...
        private class VendorPricePrompt2 : Prompt
        {
            private PlayerVendor m_Vendor;
            private VendorItem m_VI;

            public VendorPricePrompt2(PlayerVendor vendor, VendorItem vi)
            {
                m_Vendor = vendor;
                m_VI = vi;
            }
            ...
            public override void OnResponse(Mobile from, string text)
            {
                ...
                int sep = text.IndexOfAny(new char[] { ' ', ',' });
                int sep2 = sep != -1 ? text.IndexOfAny(new char[] { ' ', ',' }, sep+1) : -1;
                ...
                if (sep2 >= 0)
                    secondWord = text.Substring(sep+1, sep2-sep-1);
                else
                    secondWord = text.Substring(sep <= 0 ? 0 : sep);
                ...
                {
                    price = Convert.ToInt32(firstWord);
                    nonCitizenPrice = Convert.ToInt32(secondWord);

                    if (sep2 >= 0)
                        description = text.Substring(sep2 + 1).Trim();
                    else
                        description = "";
                }
                catch
                ...
                SetInfo(from, price, nonCitizenPrice, Utility.FixHtml(description));
            }
            ...
            private void SetInfo(Mobile from, int price, int nonCitizenPrice, string description)
            {
                ...
                if (setPrice)
                {
                    m_Vendor.SetVendorItem(item, price, nonCitizenPrice, description);
                }
                else
                {
                    m_VI.Description = description;
