...
namespace Server.Items
{
    public class TurboSlot : Item
    {
        ...
        public enum SlotThemeType
        {
            Classic, ClassicII, ClassicIII, FarmerFaceoff, GruesomeGambling, Holiday1, LadyLuck, MinerMadness, OffToTheRaces, Pirates, PowerScrolls, StatScrolls, TailorTreats, TrophyHunter
#if MINIHOUSES
			, MiniHouses
#endif
        }
        ...
        private bool m_CanDropReward = false;

        //Mobile & timeout
        private Mobile m_InUseBy = null;
        private DateTime m_LastPlayed = DateTime.Now;
        private TimeSpan m_TimeOut;
        private TimeSpan m_IdleTimer = TimeSpan.FromMinutes(2); // How long can a person be standing at the machine not playing?
        ...
        private void DoSpinWin(Mobile from, int r1index, int winnings)
        {
            ...
            m_LastPay = (int)payout;
            if (r1index < 9)
                UpdateLastWonBy(from, r1index, (int)payout);
            if (m_Profile || payout == 0)
                return;
            from.SendMessage(38, "You win {0} Gold!", (int)payout);
            if (payout >= 100000)
            {
                CommandLogging.WriteLine(from, "{0} {1} won {2} at the Casino!", from.AccessLevel, CommandLogging.Format(from), (int)payout);
            }
        }
        ...
        private bool m_DropReward()
        {
            if (!m_CanDropReward)
            {
                if (((int)((m_TotalCollected - m_TotalWon) / 200000)) > m_Iteration && ((int)((m_TotalCollected - m_TotalWon) / 200000)) > 0)
                {
                    m_Iteration = (int)((m_TotalCollected - m_TotalWon) / 200000);
                    m_CanDropReward = true;
                }
            }

            return m_CanDropReward;
        }

        private Item CreateReward(Mobile from, int m_RandReward)
        {
            Item item = null;
            switch (m_RandReward)
            {
        ...
        public void DoCashOut(Mobile from)
        {
            ...
            {
                from.BankBox.Open();
                var check = new BankCheck(m_Won);
                if (!from.BankBox.TryDropItem(from, check, false))
                    from.AddToBackpack(check);
                Timer.DelayCall(TimeSpan.FromTicks(1), () => { from.BankBox.Close(); });
            ...
            if (m_Won > 100000)
                CommandLogging.WriteLine(from, "{0} {1} Cashing out {2} from the casino", from.AccessLevel, CommandLogging.Format(from), m_Won);
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            // version 3
            writer.Write(m_Iteration);
            writer.Write(m_CanDropReward);
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            switch (version)
            {
                case 3:
                    {
                        m_Iteration = reader.ReadInt();
                        m_CanDropReward = reader.ReadBool();
                        goto case 2;
                    }
                case 2:
            ...
            if (InUseBy != null)
            {
                Timer.DelayCall(TimeSpan.FromTicks(1), ()=> { DoCashOut(InUseBy); });
            }
        }
    }
