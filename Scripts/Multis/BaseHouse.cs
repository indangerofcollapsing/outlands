...
namespace Server.Multis
{
	public abstract class BaseHouse : BaseMulti
	{
        ...
        private int m_MaxCoOwners = 15;
        [CommandProperty(AccessLevel.GameMaster)]
        public int MaxCoOwners
        {
            get { return m_MaxCoOwners; }
            set { m_MaxCoOwners = value; }
        }
        ...
        {
            get { return m_MaxFriends; }
            set { m_MaxFriends = value; }
        }

        private int m_MaxBans = 50;
        [CommandProperty(AccessLevel.GameMaster)]
        public int MaxBans
        {
            get { return m_MaxBans; }
            set { m_MaxBans = value; }
        }
		...
		public virtual TimeSpan DecayPeriod{ get{ return TimeSpan.FromDays( 45.0 ); } }
		...
		public DecayLevel GetOldDecayLevel()
		{
			TimeSpan timeAfterRefresh = DateTime.UtcNow - m_LastRefreshed;
				...
				if ( percent >= 1000 ) // 100.0%
					return DecayLevel.Collapsed;
				else if ( percent >= 977 ) // 97.7% - 99.9%
        ...
        private int m_BonusStorage = 0;
        public int BonusStorage
        {
            get { return m_BonusStorage; }
            set
            {
                m_BonusStorage = value;
                InvalidateProperties();
            }
        }
        private int m_BonusLockdowns = 0;
        public int BonusLockdowns { get { return m_BonusLockdowns; } set { m_BonusLockdowns = value; InvalidateProperties(); } }
		...
		public virtual int GetAosMaxSecures()
		{
			...
			return (int)((hpe.Storage + BonusStorage) * BonusStorageScalar);
		}
		...
		public virtual int GetAosMaxLockdowns()
		{
			...
			return (int)((hpe.Lockdowns + BonusLockdowns) * BonusStorageScalar);
		}
		...
		public void RelocateEntities(Mobile m)
		{
            var addons = new List<BaseAddon>();

			foreach ( IEntity entity in GetHouseEntities() )
			{
				Point3D relLoc = new Point3D( entity.X - this.X, entity.Y - this.Y, entity.Z - this.Z );
                if (entity is BaseAddon)
                {
                    addons.Add(entity as BaseAddon);
                    continue;
                }
            ...
            {
                if (addons[i] != null && !addons[i].Deleted)
                {
                    var deed = addons[i].Deed;
                    m.AddToBackpack(deed);
                    m.SendMessage("Your addon deed has been placed in your backpack.");
                    addons[i].Delete();
                }
            }
		}
		...
		// IPY
        public bool IsInGuardedRegion()
        {
            Region currentRegion = Region.Find(Location, Map);

            if (currentRegion != null)
                return (currentRegion.GetRegion(typeof(Server.Regions.GuardedRegion)) as Server.Regions.GuardedRegion) != null;
		...
		public bool CheckAccessibility( Item item, Mobile from )
		{
            ...
            if (!IsLockedDown(item))
                ...
                return true;
            else if (item is Runebook || item is RareStatue)
                return true;
		...
		public virtual void UpdateRegion()
		{
			...
			{
                Region currentRegion = Region.Find(Location, Map);
                Region guardedRegion = null;

                if (currentRegion != null)
                    guardedRegion = currentRegion.GetRegion(typeof(GuardedRegion)) as GuardedRegion;

                if (guardedRegion != null)
                    m_Region = new HouseRegion(this, guardedRegion);
                else
		...
		{
		 BaseDoor northDoor = MakeDoor(wood, DoorFacing.SouthCW);
		 BaseDoor southDoor = MakeDoor(wood, DoorFacing.NorthCCW);
		
			 northDoor.Locked = true;
			 southDoor.Locked = true;
		 ...
		 AddDoor(southDoor, x, y, z);
			 AddDoor(northDoor, x, y + 1, z);
		
			 return new BaseDoor[2] { southDoor, northDoor };
		}
		// IPY
		...
		public void AddSecure( Mobile m, Item item )
		{
			...
			else
			{
				...
				else
				{
                    ...
                    //Overrides For "Secure Containers" That Are Purely Decorative (Don't Want Them to Actually Take a Secure Slot)                                
                    Aquarium aquarium = item as Aquarium;
                    if (aquarium != null)
                    {
                        if (!aquarium.HouseSecureInEffect)
                        {
                            aquarium.HouseSecureInEffect = true;
                            MaxSecures++;
                        }
                    }
				...
				if ( info.Item == item && HasSecureAccess( m, info.Level ) )
				{
                    ...
                    if (item is BaseAddonContainer)
                    {
                        ...
                        if (aquarium != null)
                        {
                            if (aquarium.HouseSecureInEffect)
                            {
                                aquarium.HouseSecureInEffect = false;
                                MaxSecures--;
                            }
                        }
                    }
		...
		public override void Serialize( GenericWriter writer )
		{
            ...
            //Version 17
            writer.Write(m_MaxCoOwners);
            writer.Write(m_MaxFriends);
            writer.Write(m_MaxBans);

            //Version 16
            writer.Write(m_BonusLockdowns);
            writer.Write(m_BonusStorage);
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			{
                case 17:
                    m_MaxCoOwners = reader.ReadInt();
                    m_MaxFriends = reader.ReadInt();
                    m_MaxBans = reader.ReadInt();
                goto case 16;

                case 16:
                    BonusLockdowns = reader.ReadInt();
                    BonusStorage = reader.ReadInt();
                    goto case 15;
