...
namespace Server.Engines.Harvest
{
    ...
    {
        public static bool NewIPYMining = true;
        ...
        private Mining()
        {
            ...
            // Every bank holds from 10 to 14 ore
            // UOAC: Changed to [7, 11] for new mining
            ...
            // Mining drops to floor if pack is full.
            ...
            // Every bank holds from 6 to 12 sand ( * NoSandYo - Az * )
            // Fuck AZ: --- > UOAC: Changed to [6, 13] for sand mining
            sand.MinTotal = 6;
            sand.MaxTotal = 13;
        ...
        public override Item Construct(Type type, Mobile from, Item tool, HarvestDefinition def, HarvestBank bank, HarvestResource resource)
        {
            if (NewIPYMining)
            {
                // Ok so here's the IPY3 attempt to get around afk gathering. 
                // Enjoy
                Item item = base.Construct(type, from, tool, def, bank, resource);
                if (item == null)
                    return null;

                //If the harvest item is granite, make sure it drops only one piece.
                if (item is BaseGranite)
                    item.Amount = 1;
                else
                ...
                // Pass the harvest to the IPY harvest stash
                if (pm != null && !pm.HarvestLockedout)
                {
                    pm.TempStashedHarvestDef = def;
                    pm.TempStashedHarvest = item;		// <---- triggers a gump (HarvestGump)
