...
namespace Server.Movement
{
    ...
    {
        #region Marcis 19:46 28/03/2014
        ...
        private bool IsOk(bool ignoreDoors, bool ignoreSpellFields, int ourZ, int ourTop, StaticTile[] tiles, List<Item> items)
        {
            for (int i = 0; i < tiles.Length; ++i)
            {
                ...
                #region Marcis 19:46 28/03/2014
                //if (CheckImpassableSurfaceFlags(itemData.Flags))
            ...
            for (int i = 0; i < items.Count; ++i)
            {
                ...
                if (CheckImpassableSurfaceFlags(itemID, flags))
                #endregion
                {
            ...
            if (checkDiagonals)
            {
                ...
                for (int i = 0; i < sectors.Count; ++i)
                {
                    ...
                    for (int j = 0; j < sector.Items.Count; ++j)
                    {
                        ...
                        if (ignoreMovableImpassables && item.Movable && CheckImpassableSurfaceFlags(item.ItemID, item.ItemData.Flags))
                        #endregion
                            continue;
