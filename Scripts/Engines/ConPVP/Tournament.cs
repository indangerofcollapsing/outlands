...
namespace Server.Engines.ConPVP
{
                        ...
                        case TournamentStage.Signup:
                            {
                                ...
                                if (DisguiseTimers.IsDisguised(from))
                                {
                                    if (m_Registrar != null)
                                        m_Registrar.PrivateOverheadMessage(MessageType.Regular,
                                            0x22, false, "You cannot sign up while disguised.", from.NetState);
                                }
                                else if (from.RawStr < 75)
                                {
                                    if (m_Registrar != null)
                                        m_Registrar.PrivateOverheadMessage(MessageType.Regular,
                                            0x22, false, "You lack sufficient strength to participate in this tournament.", from.NetState);
                                }
                                else if (from.HasGump(typeof(AcceptTeamGump)))
                                {
                                    if (m_Registrar != null)
                                        m_Registrar.PrivateOverheadMessage(MessageType.Regular,
            ...
            : base(50, 50)
        {
            ...
            if (tourny.PlayersPerParticipant > 1)
            {
                ...
                for (int i = 0; i < players.Count; ++i, y += 20)
                {
                    ...
                    AddBorderedText(60, y, 200, 20, ((Mobile)players[i]).RawName, LabelColor32, BlackColor32);
                }
                            ...
                            for (int i = 0; i < m_Players.Count; ++i)
                            {
                                ...
                                else if (tourny.HasParticipant(mob))
                                {
                                    if (m_Registrar != null)
                                    {
                                        ...
                                        {
                                            m_Registrar.PrivateOverheadMessage(MessageType.Regular,
                                                0x35, false, String.Format("{0} has already entered this tournament.", mob.RawName), from.NetState);
                                        }
                                    }
                                ...
                                else if (mob is PlayerMobile && ((PlayerMobile)mob).DuelContext != null)
                                {
                                    if (m_Registrar != null)
                                    {
                                        ...
                                        {
                                            m_Registrar.PrivateOverheadMessage(MessageType.Regular,
                                                0x35, false, String.Format("{0} is already assigned to a duel. They must yield it before joining this tournament.", mob.RawName), from.NetState);
                                        }
                                    }
        ...
        private void AddPlayer_OnTarget(Mobile from, object obj)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (m_Registrar != null)
                        m_Registrar.PrivateOverheadMessage(MessageType.Regular,
                            0x59, false, String.Format("As you command m'{0}. I've given your offer to {1}.", from.Female ? "Lady" : "Lord", mob.RawName), from.NetState);
                }
            }
        }
    }
    ...
    public class AcceptTeamGump : Gump
    {
            ...
            : base(50, 50)
        {
            ...
            AddBorderedText(22, 50, 294, 40,
                String.Format("You have been asked to partner with {0} in a tournament. Do you accept?", from.RawName),
        ...
        public void AutoReject()
        {
            ...
            {
                m_Registrar.PrivateOverheadMessage(MessageType.Regular,
                    0x22, false, String.Format("{0} seems unresponsive.", m_Requested.RawName), m_From.NetState);

                m_Registrar.PrivateOverheadMessage(MessageType.Regular,
                    0x22, false, String.Format("You have declined the partnership with {0}.", m_From.RawName), m_Requested.NetState);
            }
        }
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            if (info.IsSwitched(1))
            {
                ...
                else
                {
                    ...
                    {
                        m_Registrar.PrivateOverheadMessage(MessageType.Regular,
                            0x59, false, String.Format("{0} has accepted your offer of partnership.", mob.RawName), from.NetState);

                        m_Registrar.PrivateOverheadMessage(MessageType.Regular,
                            0x59, false, String.Format("You have accepted the partnership with {0}.", from.RawName), mob.NetState);
                    }
                }
            }
            else
            {
                ...
                {
                    m_Registrar.PrivateOverheadMessage(MessageType.Regular,
                        0x22, false, String.Format("{0} has declined your offer of partnership.", mob.RawName), from.NetState);

                    m_Registrar.PrivateOverheadMessage(MessageType.Regular,
                        0x22, false, String.Format("You have declined the partnership with {0}.", from.RawName), mob.NetState);
                }
            }
        }
    }

    public class TournamentGate : Moongate
    {
        public TournamentGate()
            : base()
        {

        }

        public TournamentGate(Serial serial)
            : base(serial)
        {
    ...
    public class TournamentAutomation : Item
    {
        private static readonly int TournamentStartHour = 19; // 8pm EST

        public static TournamentAutomation Instance;

        private static TournamentController m_AutomatedInstance;
        public static TournamentController AutomatedTournament
        {
            get { return m_AutomatedInstance; }
            set
            {
                if (Instance == null || Instance.Deleted)
                    Instance = new TournamentAutomation();
                m_AutomatedInstance = value;
            }
        }

        private AutomationTimer m_Timer;

        private bool TournamentStarted;

        public TournamentAutomation()
            : base(0)
        {
            Movable = false;
            if (Instance == null || Instance.Deleted)
            {
                Instance = this;
                m_Timer = new AutomationTimer(this);
                m_Timer.Start();
        ...
        public TournamentAutomation(Serial serial)
            : base(serial)
        {
        ...
        public override void Serialize(GenericWriter writer)
        {
            base.Serialize(writer);
            writer.Write(0);
            writer.Write(m_AutomatedInstance);
        }
        ...
        public override void Deserialize(GenericReader reader)
        {
            base.Deserialize(reader);
            int version = reader.ReadInt();
            m_AutomatedInstance = reader.ReadItem() as TournamentController;
            Instance = this;
            TournamentStarted = false;
            m_Timer = new AutomationTimer(this);
            m_Timer.Start();
        ...
        private void SetupOrderRules()
        {
            m_AutomatedInstance.Tournament.Ruleset.ApplyDefault(m_AutomatedInstance.Tournament.Ruleset.Layout.Defaults[3]); // order
        }

        private void SetupChaosRules()
        {
            m_AutomatedInstance.Tournament.Ruleset.ApplyDefault(m_AutomatedInstance.Tournament.Ruleset.Layout.Defaults[4]); // chaos
        }

        private void SetupBaseRules()
        {
            m_AutomatedInstance.Tournament.SignupPeriod = TimeSpan.FromMinutes(15);
            m_AutomatedInstance.Tournament.SuddenDeath = TimeSpan.FromMinutes(15);
            m_AutomatedInstance.Tournament.SuddenDeathRounds = 100;
            m_AutomatedInstance.Tournament.TournyType = TournyType.Standard;
            m_AutomatedInstance.Tournament.FactionRestricted = false;
            m_AutomatedInstance.Tournament.GroupType = GroupingType.Random;
            m_AutomatedInstance.Tournament.ParticipantsPerMatch = 2;
            m_AutomatedInstance.Tournament.LevelRequirement = 0;
            m_AutomatedInstance.Tournament.TieType = TieType.Random;
        }

        private void StartTournament()
        {
            TournamentStarted = true;
            if (m_AutomatedInstance.Tournament.Stage == TournamentStage.Inactive)
            {
                m_AutomatedInstance.Tournament.SignupStart = DateTime.UtcNow;
                m_AutomatedInstance.Tournament.Stage = TournamentStage.Signup;
        ...
        private void OpenTournamentGates()
        {
            Moongate wbb = new TournamentGate() { Dispellable = false, Movable = false, Hue = 1122, Name = "Tournament Gate" };
            Moongate bucs = new TournamentGate() { Dispellable = false, Movable = false, Hue = 1122, Name = "Tournament Gate" };

            wbb.Target = new Point3D(5855, 1163, 0);
            wbb.TargetMap = Map.Felucca;

            wbb.MoveToWorld(new Point3D(1421, 1702, 5), Map.Felucca);

            bucs.Target = new Point3D(5855, 1163, 0);
            bucs.TargetMap = Map.Felucca;

            bucs.MoveToWorld(new Point3D(2718, 2182, 0), Map.Felucca);
        ...
        public void CheckForTournament()
        {
            if (TournamentStarted || !m_AutomatedInstance.Tournament.TournamentController.Automated)
                return;

            bool timeMatch = DateTime.Now.Hour == TournamentStartHour && DateTime.Now.Minute == 0;
            bool order = (int)DateTime.Now.DayOfWeek % 2 == 0;
        ...
        {
            TournamentStarted = false;
            Console.WriteLine("Automated Tournament has completed and closed itself down.");
        }

        private class AutomationTimer : Timer
        {
            private TournamentAutomation m_TournamentAutomation;

            public AutomationTimer(TournamentAutomation tournament)
                : base(TimeSpan.Zero, TimeSpan.FromSeconds(15))
            {
                Priority = TimerPriority.OneSecond;
                m_TournamentAutomation = tournament;
            }

            protected override void OnTick()
            {
                base.OnTick();
                if (m_TournamentAutomation == null || m_TournamentAutomation.Deleted)
                {
                    Stop();
                    return;
                }

                m_TournamentAutomation.CheckForTournament();
            }
        }
    }
    ...
    public class TournamentController : Item
    {
        ...
        {
            get { return m_Automated; }
            set
            {
                if (value)
                {
                    if (TournamentAutomation.AutomatedTournament != null)
                        TournamentAutomation.AutomatedTournament.Automated = false;

                    TournamentAutomation.AutomatedTournament = this;
                }
                m_Automated = value;
            }
        }
    ...
    public class Tournament
    {
        ...
        public TournamentController TournamentController
        {
            get
            {
                for (int i = 0; i < TournamentController.Instances.Count; i++)
                {
                    TournamentController controller = (TournamentController)TournamentController.Instances[i];
                    if (controller != null && controller.Tournament == this)
                        return controller;
                }
                return null;
            }
        ...
        {
            return m_Participants.Count * m_PlayersPerParticipant * 500;
        }
        ...
        {
            if (TournamentController != null && TournamentController.Automated && TournamentAutomation.Instance != null)
                TournamentAutomation.Instance.OnTournamentCompleted();
            ...
            else if (m_Stage == TournamentStage.Fighting)
            {
                ...
                else if (m_Pyramid.Levels.Count > 0)
                {
                    ...
                    for (int i = 0; i < m_Arenas.Count; i++)
                    {
                        ...
                        for (int j = 0; j < activeLevel.Matches.Count; j++)
                        {
                            TournyMatch match = (TournyMatch)activeLevel.Matches[j];
                            if (match.Winner == null)
                            {
                                stillGoing = true;
                                if (!match.InProgress && !arena.IsOccupied)
                                {
    ...
    public class TournyMatch
    {
        ...
        public void Start(Arena arena, Tournament tourny)
        {
            ...
            for (int i = 0; i < m_Participants.Count; ++i)
            {
                ...
                {
                    var player = (Mobile)tournyPart.Players[j];
                    if (DisguiseTimers.IsDisguised(player))
                        DisguiseTimers.RemoveTimer(player);
            ...
            if (!dc.StartedReadyCountdown)
            {
                dc.StartCountdown(45, new CountdownCallback(dc.SendReadyGump));
                m_Context = dc;
            }
            ...
            else if (!dc.StartedReadyCountdown)
            {
                dc.Unregister();
                dc.StopCountdown();
            }
