...
namespace Server.Items
{
    ...
    public abstract class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable, ISlayer, IDurability
    {
        ...
        public virtual void OnHit(Mobile attacker, Mobile defender, double damageBonus)
        {
            ...
            if (PlayerAttacker && CreatureDefender && !immuneToSpecials && !(defender is Custom.Townsystem.BaseFactionGuard))
            {
                double chance = 0.1;
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            //PlayerClass Text Override For Having Too Many Properties Displayed
            if (AccuracyLevel != WeaponAccuracyLevel.Regular && DamageLevel != WeaponDamageLevel.Regular && UOACWeaponAttribute != WeaponEnhancement.EWeaponEnhancement.None && Poison != null)
            {
                if (PlayerClassOwner != null && PlayerClass != PlayerClass.None && PlayerClassRestricted)
                    from.SendMessage("That weapon is bound to: " + PlayerClassOwner.RawName);
            }
