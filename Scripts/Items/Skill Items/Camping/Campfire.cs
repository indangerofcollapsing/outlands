...
namespace Server.Items
{
	...
	{
        public static List<Campfire> m_Instances = new List<Campfire>();
		...
		public Campfire(Mobile from) : base( 0xDE3 )
		{
            ...
            CheckExistingCampfires(from);

            m_Instances.Add(this);
		}

        public void CheckExistingCampfires(Mobile from)
        {
            if (from == null)
                return;

            Campfire oldestCampfire = null;

            int campfireCount = 0;

            for (int a = 0; a < Campfire.m_Instances.Count; a++)
            {
                Campfire campfire = Campfire.m_Instances[a];

                if (campfire == null) continue;
                if (campfire.Deleted) continue;
                if (campfire.m_CreatedBy != from) continue;

                campfireCount++;

                if (oldestCampfire == null)
                    oldestCampfire = campfire;
            }

            if (campfireCount >= 5 && oldestCampfire != null)
            {
                if (!oldestCampfire.Deleted)
                    oldestCampfire.Delete();
            }
        }
