...
namespace Server.Items
{
	public class PublicMoongate : Item
	{
        ...
        public bool UseGate(Mobile from)
		{
            ...
            if (player != null)
            {
                if (player.RecallRestrictionExpiration > DateTime.UtcNow)
                {
                    int minutes = player.RecallRestrictionExpiration.Subtract(DateTime.UtcNow).Minutes;
                    int seconds = player.RecallRestrictionExpiration.Subtract(DateTime.UtcNow).Seconds;
                    ...
                    if (sTime != "")
                        player.SendMessage("You are unable to use this moongate for another " + sTime + ".");
    ...
    public class PMEntry
	{
        ...
        public PMEntry(Point3D loc, int number):this(loc, number, 1005397) //The moongate is cloudy, and nothing can be made out. 
        {
        }
			...
			new PMList( 1012001, 1012013, Map.Felucca, new PMEntry[]
				{
					...
					new PMEntry( new Point3D( 3742, 2540, 45 ), 1076027 ),  // Ocllo
					new PMEntry( new Point3D( 3684, 1352, 0 ), 1011346 ),  // Nujel'm
					new PMEntry( new Point3D( 2959, 3532, 20 ), 1011348 ),  // Serpent's Hold
				} );
		...
		public MoongateGump( Mobile mobile, Item moongate ) : base( 100, 100 )
		{
			...
			if ( mobile.Player )
			{
				...
				else
				{
					...
					else if ( Core.AOS && (flags & ClientFlags.Tokuno) != 0 )
						checkLists = young ? PMList.AOSListsYoung : PMList.AOSLists;
					else if ( (flags & ClientFlags.Tokuno) != 0 )
			...
			AddBackground( 0, 0, 380, 370, 5054 );
