...
namespace Server.Items
{
    ...
    [FlipableAttribute(0x1BF2, 0x1BEF)]
    public class LuniteIngot : BaseIngot
    {
        [Constructable]
        public LuniteIngot(): this(1)
        {
        }

        [Constructable]
        public LuniteIngot(int amount): base(CraftResource.Lunite, amount)
        {
        }

        public LuniteIngot(Serial serial): base(serial)
        {
        }
        ...
        {
            if (Amount > 1)
                LabelTo(from, "lunite ingots : " + Amount.ToString());
            else
                LabelTo(from, "lunite ingots");
        }
