...
namespace Server.Items
{
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if ( IsForge( targeted ) )
				{
					...
					if ( from.CheckTargetSkill( SkillName.Mining, targeted, minSkill, maxSkill, 1.0 ) )
					{
    ...
    public class LuniteOre : BaseOre
    {
        [Constructable]
        public LuniteOre(): this(1)
        {
            Name = "lunite ore";
        }

        [Constructable]
        public LuniteOre(int amount): base(CraftResource.Lunite, amount)
        {
            Name = "lunite ore";
        }

        public LuniteOre(Serial serial): base(serial)
        {
        }
