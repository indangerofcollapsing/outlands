...
namespace Server.Items
{
    ...
    public abstract class BaseSiegeWeapon : BaseAddon, ISiegeWeapon
    {
        ...
        protected virtual bool FreeConsume { get; set; }
        ...
        protected XmlSiege SiegeAttachment
        {
            get
            {
        ...
        {
            LabelTo(from, string.Format("Health: [{0}/{1}]", Hits, HitsMax));
        }
        ...
        private class SiegeTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (!from.InRange(m_weapon.Location, m_weapon.MinFiringRange) || from.Map != m_weapon.Map)
                {
                    from.SendLocalizedMessage(500446); // That is too far away.
                    return;
                }
                ...
                // check if the cannon is cool enough to fire
                if (m_weapon.m_NextFiringTime > DateTime.UtcNow)
                {
                    from.SendMessage("Not ready yet.");
                    return;
                }
                ...
                if (targeted is StaticTarget)
                    m_weapon.AttackTarget(from, null, ((StaticTarget)targeted).Location, CheckLOS);
                else if (targeted is IEntity)
                    m_weapon.AttackTarget(from, (IEntity)targeted, ((IEntity)targeted).Location, true);
                else if (targeted is LandTarget)
                    m_weapon.AttackTarget(from, null, ((LandTarget)targeted).Location, CheckLOS);
