...
namespace Server.Engines.XmlSpawner2
{
	...
	{
		private const double DamageScaleFactor = 0.8; // multiplier of weapon min/max damage used to calculate siege damage.
		...
		private class HandSiegeTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				if (targeted is StaticTarget)
				{
                    ...
                    var pool = from.Map.GetItemsInRange(((StaticTarget)targeted).Location, 12);
		...
		private class InternalTimer : Timer
		{
			...
			protected override void OnTick()
			{
                ...
                if (attacker.Region is BattlegroundRegion && attacker is PlayerMobile)
                {
                    SiegeBattleground bg = ((BattlegroundRegion)attacker.Region).Battleground as SiegeBattleground;
                    if (bg.Active)
                    {
                        bg.CurrentScoreboard.UpdateScore(attacker as PlayerMobile, Categories.SiegeDamage, basedamage);
                    }
                }
