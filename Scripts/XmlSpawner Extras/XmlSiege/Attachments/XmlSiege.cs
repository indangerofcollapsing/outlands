...
namespace Server.Engines.XmlSpawner2
{
	public class XmlSiege : XmlAttachment
	{
		...
		public virtual void ApplyScaledDamage(Mobile from, int firedamage, int physicaldamage)
		{
            ...
            // prevent defense from sabotaging their walls
            if (from.Region is SiegeBattlegroundRegion)
            {
                SiegeBattleground bg = ((SiegeBattlegroundRegion)from.Region).Battleground as SiegeBattleground;
                if (bg.Active && from is PlayerMobile)
                {
                    applyDamage = !(bg.Defense.Contains(from as PlayerMobile) && (i != null && (i is SiegeWall || i is SiegeDoor)));

                    if (applyDamage)
                        bg.CurrentScoreboard.UpdateScore(from as PlayerMobile, Categories.SiegeDamage, scaleddamage);
                }
            }
			...
			if (i != null && applyDamage)
			{
				// display the damage over the target

                // UOAC hide damage done
			...
			if (targetitem is BaseMulti && targetitem.Map != null)
			{
				...
				if (mcl != null && mcl.List != null)
				{
					for (int i = 0; i < mcl.List.Length; i++)
					{
                        ...
                        eable = targetitem.Map.GetMobilesInRange(new Point3D(x, y, z), 0);
