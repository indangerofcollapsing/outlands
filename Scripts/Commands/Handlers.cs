...
namespace Server.Commands
{
	public class CommandHandlers
	{
		public static void Initialize()
		{
            ...
            //UOAC Seers on this shard can save the world (especially during the events).
			...
			Register( "Move", AccessLevel.Counselor, new CommandEventHandler( Move_OnCommand ) );
		...
		{
			BroadcastMessage( AccessLevel.Counselor, e.Mobile.SpeechHue, String.Format( "[{0}] {1}", e.Mobile.Name, e.ArgString ), true );
		}
		...
		public static void BroadcastMessage ( AccessLevel ac, int hue, string message, bool ignoreDeathcam = false ) 
		{ 
		...
		{
			e.Mobile.SendMessage( "Open Connections: {0}", Server.RemoteAdmin.ServerInfo.NetStateCount() );
