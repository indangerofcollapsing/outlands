...
namespace Server.Gumps
{
    public class AllItemGump : Gump
    {
        ...
        {
            CommandSystem.Register("AllItems", AccessLevel.Developer, new CommandEventHandler(AllItems_OnCommand));
        }
        public static string m_args;
        [Usage("AllItems")]
        [Description("Lists all items.")]
        private static void AllItems_OnCommand(CommandEventArgs e)
        {
            if (e.Length > 0)
                m_Filter = e.GetString(0).ToLower();
            else
                m_Filter = "";

            e.Mobile.SendGump(new AllItemGump(e.Mobile));
        }
        ...
        public AllItemGump(Mobile owner)
            : this(owner, BuildList(owner), 0)
        {
        }

        public AllItemGump(Mobile owner, List<Item> items, int page)
            : base(GumpOffsetX, GumpOffsetY)
        {
            owner.CloseGump(typeof(AllItemGump));
        ...
        public void Initialize(int page)
        {
            ...
            if (WipeLabel)
                AddLabel(x + WipeLabelOffsetX, y + WipeLabelOffsetY, TextHue, "Wipe All Items In This List");
        }
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                ...
                case 1: // Previous 
                    {
                        if (m_Page > 0)
                            from.SendGump(new AllItemGump(from, m_Items, m_Page - 1));
                ...
                default:
                    {
                        ...
                        if (index >= 0 && index < m_Items.Count)
                        {
                            ...
                            if (s.Deleted)
                            {
                                ...
                                from.SendGump(new AllItemGump(from, m_Items, m_Page));
                            }
                            else if (deleting)
        ...
        private class DeleteItemGump : Gump
        {
            ...
            public DeleteItemGump(Mobile from, List<Item> items, int index)
                : base(50, 50)
            {
                m_From = from;
                m_Items = items;
            ...
            public override void OnResponse(Server.Network.NetState sender, RelayInfo info)
            {
                ...
                m_From.SendGump(new AllItemGump(m_From, m_Items, 0));
            }
        }
        ...
        private class WipeAllGump : Gump
        {
            ...
            public WipeAllGump(Mobile from, List<Item> items)
                : base(50, 50)
            {
                m_From = from;
                m_Items = items;
