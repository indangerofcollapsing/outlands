...
namespace Server.Commands.Generic
{
    ...
    public class SevCommand : BaseCommand
    {
        public SevCommand()
        {
            AccessLevel = AccessLevel.Counselor;
            Supports = CommandSupport.Simple;
            Commands = new string[] { "Sev", "Sev" };
            ObjectTypes = ObjectTypes.All;
            Usage = "Sev's Command";
            Description = "Make's shit happen";
        }
        ...
        public override void Execute(CommandEventArgs e, object obj)
        {
            ...
            Effects.SendLocationParticles(EffectItem.Create(toLoc, from.Map, EffectItem.DefaultDuration), 0x3459, 10, 50, (int)EffectLayer.LeftFoot);
            from.PlaySound(0x356);
            new SevEffectTimer(1800, EffectItem.Create(toLoc, from.Map, EffectItem.DefaultDuration), 0x3709, 10, 30, (int)EffectLayer.Head, 0x208, from.Map, toLoc, 60);
    ...
    public class SevEffectTimer : Timer
    {
        EffectItem effectItem;
        int itemId, speed, duration, effect, sound;
        Map imap;
        Point3D iloc;
        int dmg;

        public SevEffectTimer(int msDelay, EffectItem ei, int nitemId, int nspeed, int nduration, int neffect, int soundid, Map map, Point3D location, int damage = 0) : base(TimeSpan.FromMilliseconds(msDelay), TimeSpan.FromMilliseconds(msDelay), 1)
        {
            effectItem = ei;
            itemId = nitemId;
            speed = nspeed;
    ...
    public class StealthSquelchCommand : BaseCommand
    {
        public StealthSquelchCommand()
        {
            AccessLevel = AccessLevel.Counselor;
            Supports = CommandSupport.AllMobiles;
            Commands = new string[] { "StealthSquelch" };
            ObjectTypes = ObjectTypes.Mobiles;
            Usage = "StealthSquelch";
            Description = "Hides the Player's Text from Other Players for 24 Hours";
        }
        ...
        public override void Execute(CommandEventArgs e, object obj)
        {
            ...
            if (state != null)
            {
                CommandLogging.WriteLine(from, "{0} {1} Stealth Squelching {2}", from.AccessLevel, CommandLogging.Format(from), CommandLogging.Format(targ));

                try
                {
                    targ.StealthSquelchedExpiration = DateTime.UtcNow + TimeSpan.FromHours(24);
                    AddResponse("They have been Stealth Squelched");
                }
    ...
    public class ClearStealthSquelchCommand : BaseCommand
    {
        public ClearStealthSquelchCommand()
        {
            AccessLevel = AccessLevel.Counselor;
            Supports = CommandSupport.AllMobiles;
            Commands = new string[] { "ClearStealthSquelch" };
            ObjectTypes = ObjectTypes.Mobiles;
            Usage = "ClearStealthSquelch";
            Description = "Clears a Stealth Squelch Set for a Player";
        }
        ...
        public override void Execute(CommandEventArgs e, object obj)
        {
            ...
            if (state != null)
            {
                CommandLogging.WriteLine(from, "{0} {1} Clearing Stealth Squelching {2}", from.AccessLevel, CommandLogging.Format(from), CommandLogging.Format(targ));

                try
                {
                    targ.StealthSquelchedExpiration = DateTime.MinValue;
                    AddResponse("They have been Cleared of Stealth Squelched");
                }
