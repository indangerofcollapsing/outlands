...
namespace Server.Commands
{
	public class Profiling
	{
		public static void Initialize()
		{
			...
			CommandSystem.Register("SetDetailedProfiles", AccessLevel.Administrator, new CommandEventHandler(SetDetailedProfiles_OnCommand));
		}
		...
		[Usage( "SetDetailedProfiles [true | false]" )]
		[Description( "Enables, disables, or toggles the detailed profiling (Custom UOAC)" )]
		public static void SetDetailedProfiles_OnCommand(CommandEventArgs e)
		{
			if ( e.Length == 1 )
				Core.DetailedProfiling = e.GetBoolean( 0 );
			else
				Core.DetailedProfiling = !Core.DetailedProfiling;

			e.Mobile.SendMessage("Detailed Profiling has been {0}.", Core.DetailedProfiling ? "enabled" : "disabled");
			if (Core.DetailedProfiling)
				Server.Custom.DetailedProfiler.OnStart();
			else
				Server.Custom.DetailedProfiler.OnEnd();
		}
