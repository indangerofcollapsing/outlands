...
namespace Server.Mobiles
{
	public class Clone : BaseCreature
	{
		...
		public Clone( Mobile caster ) : base( AIType.AI_Melee, FightMode.None, 10, 1, 0.2, 0.4 )
		{
			...
			Title = caster.Title;
			ShortTermMurders = caster.ShortTermMurders;
...
namespace Server.Mobiles
{
	public class CloneAI : BaseAI
	{
		...
		public override bool Think()
		{
			...
			Mobile master = bc_Creature.SummonMaster;

			if ( master != null && master.Map == bc_Creature.Map && master.InRange( bc_Creature, bc_Creature.RangePerception ) )
			{
				int iCurrDist = (int)bc_Creature.GetDistanceToSqrt( master );
				bool bRun = (iCurrDist > 5);

				WalkToMobile( master, 2, bRun, 0, 1 );
			}
			else
				WalkRandom( 2, 2, 1 );
