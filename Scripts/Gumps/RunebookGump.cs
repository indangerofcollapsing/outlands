...
namespace Server.Gumps
{
	public class RunebookGump : Gump
	{
		...
		private class InternalPrompt : Prompt
		{
			...
			public override void OnResponse( Mobile from, string text )
			{
				...
				else
				{
                    ...
                    if (from.Player)
                        ((Mobiles.PlayerMobile)from).CloseBankRunebookGump = false;
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( m_Book.Deleted || !from.InRange( m_Book.GetWorldLocation(), 2 ) || !Multis.DesignContext.Check( from ) || (!from.BankBox.Opened && from.BankBox.Items.Contains((Item)Book)))
			{
				m_Book.Openers.Remove( from );
                if (from.Player)
                    ((Mobiles.PlayerMobile)from).CloseBankRunebookGump = false;
