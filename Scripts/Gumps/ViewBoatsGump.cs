...
namespace Server.Gumps
{
	public class ViewBoatsGump : Gump
	{
		...
		{
			CommandSystem.Register( "ViewBoats", AccessLevel.Counselor, new CommandEventHandler( ViewBoats_OnCommand ) );
		}

		[Usage( "ViewBoats" )]
		[Description( "Displays a menu listing all boats of a targeted player." )]
		...
		public ViewBoatsGump( Mobile from, List<BaseBoat> list, BaseBoat sel ) : base( 50, 40 )
		{
			...
			if ( sel == null || sel.Deleted )
			{
				...
				for ( int i = 0; i < list.Count; ++i )
				{
					...
					object name = FindBoatName( list[i] );
			...
			else
			{
                ...
                houseName = (sel.ShipName == null) ? "An Unnamed Boat" : sel.ShipName;
				...
				AddHtml( 15, 140, 210, 20, Color( "Hit Points:", White ), false, false );
				AddHtml( 15, 140, 210, 20, Color( Right( sel.HitPoints.ToString() ), White ), false, false );

				AddHtml( 15, 160, 210, 20, Color( "Max Hit Points:", White ), false, false );
				AddHtml( 15, 160, 210, 20, Color( Right( sel.MaxHitPoints.ToString() ), White ), false, false );

				AddHtml( 15, 180, 210, 20, Color( "Cannons:", White ), false, false );
				AddHtml( 15, 180, 210, 20, Color( Right( sel.Cannons.Count.ToString() ), White ), false, false );

				AddHtml( 15, 200, 210, 20, Color( "Decays:", White ), false, false );
				AddHtml( 15, 200, 210, 20, Color( Right( "Yes" ), White ), false, false );

				AddHtml( 15, 220, 210, 20, Color( "Decay Time:", White ), false, false );
				AddHtml( 15, 220, 210, 20, Color( Right( sel.DecayTime.ToString() ), White ), false, false );
				...
				AddHtml( 50, 265, 120, 20, Color( "Demolish boat", White ), false, false );

				AddButton( 15, 285, 4005, 4007, 3, GumpButtonType.Reply, 0 );
				AddHtml( 50, 285, 120, 20, Color( "Refresh boat", White ), false, false );
		...
		public override void OnResponse( Server.Network.NetState sender, RelayInfo info )
		{
			if ( m_Selection == null )
			{
				...
				if ( v >= 0 && v < m_List.Count )
					m_From.SendGump( new ViewBoatsGump( m_From, m_List, m_List[v] ) );
			}
			else if ( !m_Selection.Deleted )
			{
				switch ( info.ButtonID )
				{
					...
					case 2:
					{
                        ...
                        {
                            m_From.SendMessage("Counselors are not allowed to delete items.");
                            return;
                        }
		...
		public object FindBoatName( BaseBoat house )
		{
