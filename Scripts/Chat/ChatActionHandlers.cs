...
namespace Server.Engines.Chat
{
	public class ChatActionHandlers
	{
		...
		static ChatActionHandlers()
		{
            ...
            Register(0x43, false, false, new OnChatAction( LeaveRequest ));
        ...
        public static void LeaveRequest(ChatUser from, Channel channel, string param)
        {
            if (from != null && from.CurrentChannel != null)
            {
                from.Mobile.SendMessage("You have left the channel '{0}'.", from.CurrentChannel.Name);
                from.CurrentChannel.RemoveUser(from);
            }
        }
