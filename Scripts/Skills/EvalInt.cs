...
namespace Server.SkillHandlers
{
	public class EvalInt
	{
		...
		{
            SkillInfo.Table[(int)SkillName.EvalInt].Callback = new SkillUseCallback(OnUse);
		}
		...
		public static TimeSpan OnUse( Mobile m )
		{
			...
			return TimeSpan.FromSeconds( 1.77 );
		}
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( targeted is Mobile )
				{
                    ...
                    from.NextSkillTime = Core.TickCount + (int)(SkillCooldown.EvalIntCooldown * 1000);

					if ( from.CheckTargetSkill( SkillName.EvalInt, targ, 0.0, 100.0, 1.0 ) )
					{
