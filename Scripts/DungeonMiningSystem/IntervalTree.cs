...
namespace IntervalTree
{
    ...
    {        
        ...
        public List<KeyValuePair<Interval<T>, TypeValue>> GetIntervalsEndingAt(T arg)
        {
            return IntervalNode.GetIntervalsEndingAt(this.Root, arg);
        }
        ...
        private class IntervalNode
        {
            ...
            public static List<KeyValuePair<Interval<T>, TypeValue>> GetIntervalsEndingAt(IntervalNode subtree, T end)
            {
                if (subtree != null)
                {
                    int compareResult = end.CompareTo(subtree.Interval.End);
                    if (compareResult < 0)
                    {
                        return GetIntervalsEndingAt(subtree.Left, end);
                    }
                    else if (compareResult > 0)
                    {
                        return GetIntervalsEndingAt(subtree.Right, end);
                    }
                    else
                    {
                        var result = new List<KeyValuePair<Interval<T>, TypeValue>>();
