...
namespace Server
{
    ...
    public class DamageEntry
    {
        ...
        public bool HasExpired { get { return (DateTime.UtcNow > (m_LastDamage + m_Damager.DamageEntryExpiration)); } }
    ...
    public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable, ISpawnable
    {
        ...
        public Timer m_LogoutTimer;
        private Timer m_CombatTimer;

        public Timer m_ManaTimer, m_HitsTimer, m_StamTimer;
        ...
        public StealthMovementTimer m_StealthMovementTimer;
        public StealthAttackTimer m_StealthAttackTimer;

        private DateTime m_StealthAttackExpiration;        
        private bool m_StealthAttackReady;
        ...
        public DateTime NextBoardingAttemptAllowed
        {
            get { return m_NextBoardingAttemptAllowed; }
            set
            {
                m_NextBoardingAttemptAllowed = value;
            }
        }

        #region BLUE_HEALING_ORANGE
        private class ExpireAssistedOwnMilitiaTimer : Timer
        {
            private Mobile m_Mobile;

            public ExpireAssistedOwnMilitiaTimer(Mobile m)
            ...
            {
                m_Mobile.AssistedOwnMilitia = false;
            }
        }

        private bool m_AssistedOwnMilitia;
        private Timer m_AssistedOwnMilitiaTimer;
        [CommandProperty(AccessLevel.Counselor, AccessLevel.GameMaster)]
        public bool AssistedOwnMilitia
        {
            get
            {
                return m_AssistedOwnMilitia;
            }
            set
            {
                if (m_AssistedOwnMilitia != value)
                {
                    m_AssistedOwnMilitia = value;
                    Delta(MobileDelta.Noto);
                    InvalidateProperties();
                    //ValidateGhostVisibility(); // hack to make sure other people's noto highlight is updated from this mobile's perspective
                }

                if (m_AssistedOwnMilitia)
                {
                    if (m_AssistedOwnMilitiaTimer == null)
                        m_AssistedOwnMilitiaTimer = new ExpireAssistedOwnMilitiaTimer(this);
                    else
                        m_AssistedOwnMilitiaTimer.Stop();

                    m_AssistedOwnMilitiaTimer.Start();
                }
                else if (m_AssistedOwnMilitiaTimer != null)
                {
                    m_AssistedOwnMilitiaTimer.Stop();
                    m_AssistedOwnMilitiaTimer = null;
                }
            }
        }
        #endregion

        public bool IncomingResistCheckFromMonster; // hack, double resist gains when spell cast by monster.
        private DateTime m_LastStolenItem;
        public DateTime LastStolenItem
        {
            get { return m_LastStolenItem; }
            set { m_LastStolenItem = value; }
        }
        ...
        private bool m_BeingBandaged = false;

        public virtual int AttackRange { get { return 1; } }

        public virtual bool AllowTrades { get { return true; } }

        public virtual TimeSpan DamageEntryExpiration { get { return TimeSpan.FromMinutes(2); } }

        [CommandProperty(AccessLevel.GameMaster, AccessLevel.Administrator)]
        public bool BeingBandaged
        {
            get { return m_BeingBandaged; }
            set
            {
                m_BeingBandaged = value;
            }
        }
        ...
        private bool m_combatProhibited;
        ...
        public double BackstabDamageScalar = 1.0;        

        [CommandProperty(AccessLevel.GameMaster)]
        public bool StealthAttackReady
        {
            get
            {
                return m_StealthAttackReady;
            }
            set
            {
                m_StealthAttackReady = value;
            }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public bool StealthAttackActive
        {
            get
            {
                return m_StealthAttackActive;
            }
            set
            {
                m_StealthAttackActive = value;
            }
        }

        private double m_DispelResist = 0;

        [CommandProperty(AccessLevel.GameMaster)]
        public Double DispelResist
        {
            get { return m_DispelResist; }
            set { m_DispelResist = value; }
        }
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public bool CombatProhibited
        {
            get { return m_combatProhibited; }
            set { m_combatProhibited = value; }
        }
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public virtual bool RevealImmune { get { return m_RevealImmune; } set { m_RevealImmune = value; } }
        ...
        public void DoStealthAttack(double duration)
        {
            m_StealthAttackActive = true;
            StealthAttackReady = false;

            m_StealthAttackExpiration = DateTime.UtcNow + TimeSpan.FromSeconds(duration);

            m_StealthAttackTimer = new StealthAttackTimer(this, TimeSpan.FromSeconds(duration));
            m_StealthAttackTimer.Start();
        }

        public void CancelStealthAttack(double duration)
        {
            m_StealthAttackActive = false;
            StealthAttackReady = false;

            m_StealthAttackExpiration = DateTime.UtcNow;
        }

        public virtual bool Paralyze(Mobile from, double duration)
        {
            if (!m_Paralyzed)
            {
                Paralyzed = true;

                m_ParaTimer = new ParalyzedTimer(this, TimeSpan.FromSeconds(duration));
                m_ParaTimer.Start();
        ...
        private static int m_ActionMessageDelay = 125; //millisec
        ...
        public ILogoutRetain FindLogoutRetainer()
        {
            ...
            Sector sector = m_Map.GetSector(m_Location);
            ...
            {
                BaseMulti retainer = sector.Multis[i] as BaseMulti;

                if (retainer != null && retainer is ILogoutRetain && retainer.Contains(X, Y))
                    return ((ILogoutRetain)retainer);
            }
        ...
        private class LogoutTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                if (m_Mobile.m_Map != Map.Internal)
                {
                    ...
                    ILogoutRetain retainer = m_Mobile.FindLogoutRetainer();

                    if (retainer != null)
                        m_Mobile.m_RetainedLogout = retainer;
        ...
        public class HidingTimer : Timer
        {
            public Mobile m_Mobile;
            public DateTime m_StartTime;
            public bool m_IsStealthing;

            public HidingTimer(Mobile mobile, DateTime startTime, bool isStealthing): base(TimeSpan.FromSeconds(10))
            {
                m_Mobile = mobile;
                m_StartTime = startTime;
                m_IsStealthing = isStealthing;
            ...
            protected override void OnTick()
            {
                if (!m_IsStealthing)
                {
                    if (m_Mobile.Skills[SkillName.Stealth].Value >= 50 && m_Mobile.Skills[SkillName.Stealth].Value >= 80)
                    {
                        if (m_Mobile.Hidden)
                            m_Mobile.SendStealthReadyNotification();                            
                    }
                }
        ...
        public class StealthMovementTimer : Timer
        {
            public Mobile m_Mobile;

            public StealthMovementTimer(Mobile mobile): base(TimeSpan.FromSeconds(10))
            {
                m_Mobile = mobile;
                Priority = TimerPriority.FiftyMS;
            }
            ...
            protected override void OnTick()
            {
                if (m_Mobile.Skills[SkillName.Stealth].Value >= 50 && m_Mobile.Skills[SkillName.Stealth].Value >= 80)
                {
                    if (m_Mobile.Hidden && m_Mobile.StealthAttackReady)
                        m_Mobile.SendStealthMovementReadyNotification();
                }   
        ...
        public virtual void SendStealthReadyNotification()
        {
            SendMessage("You feel comfortable enough to begin stealthing.");
        }

        public virtual void SendStealthMovementReadyNotification()
        {
            SendMessage("You feel ready to continue stealthing.");
        }

        public class StealthAttackTimer : Timer
        {
            private Mobile m_Mobile;

            public StealthAttackTimer(Mobile m, TimeSpan duration)
                : base(duration)
            {
            ...
            protected override void OnTick()
            {
                if (DateTime.UtcNow >= m_Mobile.m_StealthAttackExpiration)
                {
                    m_Mobile.StealthAttackActive = false;
                    m_Mobile.StealthAttackReady = false;

                    m_Mobile.m_StealthAttackTimer.Stop();
                }
            }
        }
        ...
        public virtual bool IgnoreHinderForSwings { get { return false; } }

        public virtual bool IsHindered()
        {
            return false;
        }
        ...
        {
            // 11/13/13 - Adding a condition to disallow ranged weapons from being used in
            // arena pre-match.
            if (CombatProhibited)
            {
                this.SendMessage("That action is temporarily disabled.");
                return false;
            }
        ...
        {
            AggressiveAction(aggressor, false, true);
        }

        public virtual void AggressiveAction(Mobile aggressor, bool criminal, bool causeCombat)
        {
            ...
            if (addAggressor)
            {
                ...
                PushNotoriety(this, aggressor, true);
        ...
        public void RemoveAggressed(Mobile aggressed)
        {
            ...
            for (int i = 0; i < list.Count; ++i)
            {
                ...
                if (info.Defender == aggressed)
                {
                    ...
                    PushNotoriety(this, aggressed, false);
        ...
        public DateTime NextResyncMovementClearFastWalkAllowed = DateTime.UtcNow;
        public int ResyncsOccuredInTimeWindow = 0;
        public static int ResyncsNeccessaryForStaffNotification = 1;
        public DateTime NextFastResyncStaffNotification = DateTime.UtcNow;        
        ...
        {
            get { return m_StamFreeMoveExpiration; }
            set { m_StamFreeMoveExpiration = value; }
        }

        private Point3D m_StamFreeMoveSource;
        [CommandProperty(AccessLevel.GameMaster)]
        public Point3D StamFreeMoveSource
        {
            get { return m_StamFreeMoveSource; }
            set { m_StamFreeMoveSource = value; }
        } 
        ...
        public virtual void OnAfterDelete()
        {
            ...
            if (m_StealthMovementTimer != null)
                m_StealthMovementTimer.Stop();            
        ...
        public virtual bool OnBeforeHarmfulSpell()
        {
            return true;
        }
        ...
        public DateTime m_NextCustomAlchemyPositiveEffectAllowed = DateTime.MinValue;
        [CommandProperty(AccessLevel.GameMaster, AccessLevel.GameMaster)]
        public DateTime NextCustomAlchemyPositiveEffectAllowed
        {
            get { return m_NextCustomAlchemyPositiveEffectAllowed; }
            set { m_NextCustomAlchemyPositiveEffectAllowed = value; }
        }

        public DateTime m_LastCustomAlchemyPositiveEffect = DateTime.MinValue;
        [CommandProperty(AccessLevel.GameMaster, AccessLevel.GameMaster)]
        public DateTime LastCustomAlchemyPositiveEffect
        {
            get { return m_LastCustomAlchemyPositiveEffect; }
            set { m_LastCustomAlchemyPositiveEffect = value; }
        }

        public DateTime m_LastCustomAlchemyNegativeEffect = DateTime.MinValue;
        [CommandProperty(AccessLevel.GameMaster, AccessLevel.GameMaster)]
        public DateTime LastCustomAlchemyNegativeEffect
        {
            get { return m_LastCustomAlchemyNegativeEffect; }
            set { m_LastCustomAlchemyNegativeEffect = value; }
        }

        public bool m_HideSkillUpdates = false;
        [CommandProperty(AccessLevel.GameMaster, AccessLevel.GameMaster)]
        public bool HideSkillUpdates
        {
            get { return m_HideSkillUpdates; }
            set { m_HideSkillUpdates = value; }
        }

        public int m_AggroBonus = 0;
        [CommandProperty(AccessLevel.GameMaster, AccessLevel.Seer)]
        public int AggroBonus
        {
            get { return m_AggroBonus; }
            set { m_AggroBonus = value; }
        }

        public DateTime m_AggroBonusExpiration = DateTime.MinValue;
        [CommandProperty(AccessLevel.GameMaster, AccessLevel.Seer)]
        public DateTime AggroBonusExpiration
        {
            get { return m_AggroBonusExpiration; }
        ...
        private DateTime m_LastPlayerCombatTime = DateTime.MinValue;
        [CommandProperty(AccessLevel.GameMaster)]
        public DateTime LastPlayerCombatTime
        {
            get { return m_LastPlayerCombatTime; }
            set
            { 
                m_LastPlayerCombatTime = value;
                LastPlayerCombatTimeChanged();
            }
        }
            ...
            if (m_Map != null)
            {
                ...
                foreach (IEntity o in eable)
                {
                    if (o is Mobile)
                    {
                        ...
                        // Xiani - Adding IgnoreLos to provide the ArenaRegion an opportunity to override the los
                        // requirement. The stock speech args allows you to explicitly block speech, not explitly
                        // allow speech.

                        if (heard.CanSee(this) && (regArgs.IgnoreLos || m_NoSpeechLOS || !heard.Player || heard.InLOS(this)))
                        {
                ...
                for (int i = 0; i < onSpeech.Count; ++i)
                {
                    ...
                    if (obj is Mobile)
                    {
                        ...
                        if (heard.CheckCreatureOwnership(this))
                            heard.OnSpeech(regArgs);
        ...
        public virtual bool CheckCreatureOwnership(Mobile target)
        {
            return false;
        }
        ...
        public virtual bool CheckDisrupt(int amount, Mobile from)
        {
            return true;
        }
        ...
        public virtual void Damage(int amount, Mobile from, bool informMount)
        {
            ...
            if (amount > 0)
            {
                bool willDisrupt = CheckDisrupt(amount, from);

                int oldHits = Hits;
                int newHits = oldHits - amount;

                if (m_Spell != null && willDisrupt)
        ...
        public void UsedStuckMenu()
        {
            ...
            for (int i = 0; i < m_StuckMenuUses.Length; ++i)
            {
                if ((DateTime.UtcNow - m_StuckMenuUses[i]) > TimeSpan.FromHours(12))
                {
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public DateTime StealthSquelchedExpiration
        {
            get
            {
                return m_StealthSquelchedExpiration;
            }
            set
            {
                m_StealthSquelchedExpiration = value;
            }
        }
        ...
        public virtual void Deserialize(GenericReader reader)
        {
            ...
            {
                case 40:
                    m_HideSkillUpdates = reader.ReadBool();
                goto case 39;

                 case 39:
                    m_AggroBonus = reader.ReadInt();
                    m_AggroBonusExpiration = reader.ReadDateTime();
                    m_LastCombatTime = reader.ReadDateTime();
                    m_LastPlayerCombatTime = reader.ReadDateTime();
                 goto case 38;
                ...
                case 37:
                    m_StamFreeMoveExpiration = reader.ReadDateTime();
                    m_StamFreeMoveSource = reader.ReadPoint3D();
                ...
                case 33:
                    {
                        m_DispelResist = reader.ReadDouble();
                ...
                case 32:
                    {
                        m_RetainedLogout = (ILogoutRetain)reader.ReadItem();
                        goto case 31;
                    }
                case 31:
                    {
                ...
                case 0:
                    {
                        ...
                        {
                            ILogoutRetain retainer = FindLogoutRetainer();

                            if (retainer != null)
                                m_RetainedLogout = retainer;
        ...
        public virtual void Serialize(GenericWriter writer)
        {
            ...
            //Version 40
            writer.Write(m_HideSkillUpdates);

            //Version 39
            writer.Write(m_AggroBonus);
            writer.Write(m_AggroBonusExpiration);
            writer.Write(m_LastCombatTime);
            writer.Write(m_LastPlayerCombatTime);
            ...
            //Version 37
            writer.Write(m_StamFreeMoveExpiration);
            ...
            //version 33
            writer.Write((double)m_DispelResist);
        ...
        {
            return (Body.IsHuman || Body.IsGhost || IsBodyMod || this.Player);
        }
        ...
        public virtual bool CanHear(Mobile from)
        {
            return true;
        }
        ...
        public virtual void DoHarmful(Mobile target, bool indirect)
        {
            ...
            target.AggressiveAction(this, isCriminal, true);
        ...
        public void RemoveStatModsBeginningWith(string str)
        {
            bool result = false;
            for (int i = m_StatMods.Count - 1; i >= 0; --i)
            {
                StatMod check = m_StatMods[i];
                if (check.Name.StartsWith(str))
                {
        ...
        ILogoutRetain m_RetainedLogout;
            ...
            set
            {
                if (m_NetState != value)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (m_RetainedLogout != null && (!(m_RetainedLogout is Item) || !((Item)m_RetainedLogout).Deleted))
                            {
                                Map loginMap = m_RetainedLogout.LoginMap;

                                if (loginMap == null || loginMap == Map.Internal)
                                    loginMap = Map.Felucca;

                                Map = loginMap;
                                Location = m_RetainedLogout.LoginLocation;

                                m_RetainedLogout = null;
                            }
                            else
                            {
                                Map = m_LogoutMap;
                ...
                if (map != null)
                {
                    ...
                    foreach (NetState ns in eable)
                    {
                        if (ns != m_NetState && !Utility.InUpdateRange(newLocation, ns.Mobile.Location))
                        {
                            ...
                            /* IPY STICKYBAR FIX */
                            //StickyBar.Queue(this, ns.Mobile);
                            /* STICKYBAR FIX */
                        }
                    }
        ...
        {
            // 11/13/13 - Adding a condition to disallow any item use during arena pre-match.
            if (CombatProhibited)
            {
                this.SendMessage("That action is temporarily disabled.");
                return false;
        ...
        public Point3D GetPointByDirection(Point3D start, Direction direction)
        {
            Point3D newPoint = start;
        ...
        public virtual bool RangeExemption(Mobile combatant)
        {
            return false;
        }
