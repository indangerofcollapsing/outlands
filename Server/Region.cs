...
namespace Server
{
    ...
    public class Region : IComparable
	{
		...
		public IndexedRegionName IndexedName { get { return m_IndexedName; } }

		public Map Map { get { return m_Map; } set { m_Map = value; } }
        public Rectangle3D[] Area { get { return m_Area; } set { m_Area = value; } }
		...
		public delegate void PlayerInRegionDelegate(Mobile mob);
		public void ForEachPlayerInRegion( PlayerInRegionDelegate callback )
		{
			if ( m_Sectors != null )
			{
				for ( int i = 0; i < m_Sectors.Length; i++ )
		...
		public int GetPlayerCount(bool player_access_level_only = false)
		{
			int count = 0;
			...
			if ( m_Sectors != null )
			{
				for ( int i = 0; i < m_Sectors.Length; i++ )
				{
					...
					foreach ( Mobile player in sector.Players )
					{
						if (player_access_level_only && player.AccessLevel > AccessLevel.Player)
							continue;
