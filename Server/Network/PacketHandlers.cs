...
namespace Server.Network
{
	...
	public static class PacketHandlers
	{
		...
		public static void RenameRequest( NetState state, PacketReader pvSrc )
		{
			...
			if ( targ != null )
				EventSink.InvokeRenameRequest( new RenameRequestEventArgs( from, targ, pvSrc.ReadStringSafe().ToString() ) );
		}
		...
		public static void AsciiPromptResponse( NetState state, PacketReader pvSrc )
		{
			...
			string text = pvSrc.ReadStringSafe().ToString();
		...
		public static void Resynchronize( NetState state, PacketReader pvSrc )
		{
            ...
            bool UseResyncMovementFix = false;

            //Resync Movement Fix
            if (!UseResyncMovementFix)
            {
                state.Sequence = 0;
            ...
            {
                TimeSpan ResynchMovementRequiredCooldown = TimeSpan.FromSeconds(30);
                TimeSpan StaffNotificationCooldown = TimeSpan.FromSeconds(30);

                if (m.AccessLevel == AccessLevel.Player && m.Player)
                {
                    if (DateTime.UtcNow >= m.NextResyncMovementClearFastWalkAllowed)
                    {
                        m.NextResyncMovementClearFastWalkAllowed = DateTime.UtcNow + ResynchMovementRequiredCooldown;
                        m.ResyncsOccuredInTimeWindow = 0;
                    ...
                    {
                        m.ResyncsOccuredInTimeWindow++;

                        if (m.ResyncsOccuredInTimeWindow >= Mobile.ResyncsNeccessaryForStaffNotification)
                        {
                            if (DateTime.UtcNow >= m.NextFastResyncStaffNotification)
                            {
                                foreach (NetState netstate in NetState.Instances)
                                {
                                    ...
                                    mobile.SendMessage(2115, "Fast Resync Detected for: " + m.RawName + "\n");
                                }

                                m.NextFastResyncStaffNotification = DateTime.UtcNow + StaffNotificationCooldown;
                            }
                        }
                    }
                }
		...
		public static void AsciiSpeech( NetState state, PacketReader pvSrc )
		{
            ...
            StringBuilder text = pvSrc.ReadStringSafe();
            ...
            //    SpeechFilter.Filter(ref text);

			from.DoSpeech( text.ToString(), m_EmptyInts, type, Utility.ClipDyedHue( hue ) );
		}
		...
		public static void UnicodeSpeech( NetState state, PacketReader pvSrc )
		{
			...
			from.DoSpeech( text.ToString(), keywords, type, Utility.ClipDyedHue( hue ) );
		}
		...
		{
			if ( PartyCommands.Handler != null )
				PartyCommands.Handler.OnPrivateMessage( state.Mobile, World.FindMobile( pvSrc.ReadInt32() ), pvSrc.ReadUnicodeStringSafe().ToString() );
		}

		public static void PartyMessage_PublicMessage( NetState state, PacketReader pvSrc )
		{
			if ( PartyCommands.Handler != null )
                PartyCommands.Handler.OnPublicMessage(state.Mobile, pvSrc.ReadUnicodeStringSafe().ToString());
		}
